...
int sm2_gen_key(const char *pri_key, char *pub_key_x, char *pub_key_y)
{
    ...
    BIGNUM *p, *a, *b, *gx, *gy, *z, *p_private;
    ...
    EC_POINT_mul(group, point_p, p_private, NULL, NULL, NULL);
    EC_KEY_set_private_key(key, p_private);
    EC_KEY_set_public_key(key, point_p);
    
    //    point_q = EC_KEY_get0_public_key(key);
    if(!EC_POINT_get_affine_coordinates_GFp(group, point_p, gx, gy , NULL))
    {
        goto err_process;
    }
    char *pub_x = BN_bn2hex(gx);
    char *pub_y = BN_bn2hex(gy);
    memcpy(pub_key_x, pub_x, 64);
    memcpy(pub_key_y, pub_y, 64);
//    pub_key_x = BN_bn2hex(gx);
//    pub_key_y = BN_bn2hex(gy);
